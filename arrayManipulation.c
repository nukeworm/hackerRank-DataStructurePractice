...
long long arrayManipulation(long long n, long long queries_rows, long long queries_columns, long long** queries) {
    long long max=0;
    long long arr[n];
    for(long long i=0 ; i<n ; i++)
        arr[i]=0;
    for(long long i=0 ; i<queries_rows ; i++){
...
int main()
{
    ...
    char* n_endptr;
    char* n_str = nm[0];
    long long n = strtol(n_str, &n_endptr, 10);

    if (n_endptr == n_str || *n_endptr != '\0') { exit(EXIT_FAILURE); }

    char* m_endptr;
    char* m_str = nm[1];
    long long m = strtol(m_str, &m_endptr, 10);

    if (m_endptr == m_str || *m_endptr != '\0') { exit(EXIT_FAILURE); }

    long long** queries = malloc(m * sizeof(int*));
    ...
    long long queries_rows = m;
    int queries_columns = 3;

    long long result = arrayManipulation(n, queries_rows, queries_columns, queries);
