...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    //taking limit=100 for total appends in seqList
    int seqList[n][100],lastAnswer=0,lA=0,seq,seqSize[n],size,val;
    int* lastA=(int *)malloc(queries_rows * sizeof(int));
    ...
    for(int i=0;i<queries_rows;i++){
        ...
        if(queries[i][0]==1){
            ...
            size=seqSize[seq];
            seqSize[seq]++;
            seqList[seq][size]=queries[i][2];
        }
        //Query Type:2
        else{
            seq=(queries[i][1]^lastAnswer)%n;
            size=seqSize[seq];
            val=queries[i][2]%size;
